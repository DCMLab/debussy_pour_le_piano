...
Each piece in this corpus is represented by five files with identical names, each in its own folder. For example, the *Prélude* has the following files:

-  ``MS3/l095-01_pour_prelude.mscx``: Uncompressed MuseScore 3.6.2 file including the music and annotation labels.
-  ``notes/l095-01_pour_prelude.notes.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/l095-01_pour_prelude.measures.tsv``: A table with relevant information about the measures in the score.
-  ``chords/l095-01_pour_prelude.chords.tsv``: A table containing layer-wise unique onset positions with the musical markup (such as dynamics, articulation, lyrics, figured bass, etc.).
-  ``harmonies/l095-01_pour_prelude.harmonies.tsv``: A table of the included harmony labels (including cadences and phrases) with their positions in the score.
